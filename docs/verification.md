...
- `KUJO_BIN=... bash scripts/run_ci_checks.sh` in Kujo SSG — CLI contracts, generated contracts, build, and HTML validation passed.
...
- `python3 scripts/validate_site.py output` — 195 primary routes checked; titles, descriptions, canonicals, one-H1 structure, unique IDs, alt attributes, JSON-LD, assets, required routes, and internal targets validated; passed with 73 historical-link warnings.
- Browser QA at 1440px and 375–390px — homepage, writing index, representative and long-form articles, projects, about, and 404 rendered with one H1, a main landmark, and no horizontal overflow. Evidence is in `reports/visual/`.

## Representative coverage

Verified routes include homepage, writing and pagination, root-level article, long/code-heavy articles, projects listing/item, about, contact, category/tag archives, legacy service route, 404, feed, sitemap, robots, and llms.txt. Source CSS includes 64rem, 48rem, reduced-motion, forced-colors, and print paths. The mobile navigation is native HTML and remains functional without JavaScript.
